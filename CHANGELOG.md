...
- **docs**: Sprint 35 retro escape closure -- new mandatory Phase 6.4.5 "Convert PR from draft to ready for review" in `docs/SPRINT_EXECUTION_WORKFLOW.md` and corresponding `docs/SPRINT_CHECKLIST.md` checkbox. PR #238 stayed in draft through Phase 6 and Phase 7 close-out because the prior workflow only carried an advisory "Note" at Phase 3.3.1 ("convert when Phase 5.2 tests pass"). Harold had to manually surface the issue blocking merge. Step is now enumerated and mandatory: `gh pr ready <PR-number>`, verified with `gh pr view <PR-number> --json isDraft,mergeable`. Standing Approval Inventory (Phase 3.7) updated to include `gh pr ready` as plan-approved. Model-Version Pitfalls appendix in CLAUDE.md gains a 6th Opus 4.7 entry covering this pattern (Issue #237)
