...
- **fix**: BUG-S40-1 -- delete and move actions on AOL no longer report false successes. The IMAP delete/move path issues `UID MOVE` to the target folder and previously marked every message succeeded if the command did not throw. AOL acknowledges `UID MOVE` with `OK` but silently retains spam-classified messages in the source folder (the "copy-not-move" pathology, here as true survivors with the SAME UID, distinct from F91's re-injected new-UID copies). `GenericIMAPAdapter.moveToFolderBatch` now verifies the move landed: after the batch move it re-searches the source folder (`UID SEARCH`) for the moved UIDs, retries any survivors once, and records messages that still remain as genuine FAILURES instead of successes. This fixes a delete-loop where the same messages were "deleted" on every scan and reappeared each time (observed 2026-06-03 on `kimmeyharold@aol.com`: scan 1 reported 482 deleted but 271 of them remained in Bulk Mail and were re-deleted on scan 2). New pure helper `partitionByMoveSurvival` (5 unit tests) plus `_uidsStillPresent` verification search. Affects delete, moveToJunk, and safe-sender move (all route through `moveToFolderBatch`); non-AOL providers are unaffected in the common case where the move succeeds on the first try. (BUG-S40-1)
