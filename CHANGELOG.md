...
- **test**: F97 -- re-authored the 2 deferred F56 create+delete lifecycle scripts (`test_f56_create_block_rule.json`, `test_f56_create_safe_sender.json`) to the current `testCases` schema, with the Add-Block-Rule input format confirmed against the live Sprint-40 rule-creation UI (3 live-verified selector fixes). Both run unattended with self-cleaning lifecycle (zero net DB drift). (F97)
- **test**: F76 -- added layout-bounds visual-regression checking to the WinWright sweep. New `winwright-visual-check.ps1` compares the live accessibility-tree selector bounds for the primary screens (home, manage rules, settings/general) against committed baseline JSON under `test/winwright/baselines/`, within a configurable pixel tolerance (default 8px). Wired into `run-winwright-tests.ps1` via `-VisualCheck`; a missing baseline prints `[PENDING CAPTURE]` and does not fail the run. (F76)
