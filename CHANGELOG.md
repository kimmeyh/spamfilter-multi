...
### 2026-05-01 (Sprint 37 -- Phase 7 retrospective improvements)
- **feat**: Manage Rules and Manage Safe Senders list rows are now selectable for copy. Replaced `Text` with `SelectableText` in row title + subtitle on both screens. Tap on the leading category icon still opens the rule/sender details dialog; tap on the row text now selects the text instead. (Sprint 37 Phase 7 Imp-1, Harold Category 1 feedback)
- **feat**: New Help screen section "Other ways to reduce junk email, mail, texts, and phone calls" at the bottom of the Help screen. Sub-sections for unwanted emails (FTC, APWG `reportphishing@apwg.org`), unwanted texts (forward to `7726`, FTC `ReportFraud.ftc.gov`), unwanted postal mail (`OptOutPrescreen.com`, `DMAchoice.org`, `CatalogChoice.org`), and unwanted phone calls (`DoNotCall.gov`, `1-888-382-1222`, carrier spam-call services). Footer timestamp bumped to "Sprint 37 (May 2026)". 3 new widget tests covering section presence, key reporting destinations, and footer timestamp. (Sprint 37 Phase 7 Imp-2, Harold Category 13 feedback)
