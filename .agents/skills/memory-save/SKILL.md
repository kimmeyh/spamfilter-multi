...
- **Memory file**: `D:/Data/Harold/github/spamfilter-multi/.Codex/memory/current.md`
- **Metadata file**: `D:/Data/Harold/github/spamfilter-multi/.Codex/memory/memory_metadata.json`

DO NOT use relative paths like `.Codex/memory/current.md` — this can resolve to different directories depending on context.

## CRITICAL: Writing to .Codex/ paths

The Write tool is BLOCKED for `.Codex/` paths in don't-ask mode. You MUST use one of these alternatives:

**Preferred — Python** (handles markdown with backticks cleanly):
```bash
python << 'EOF'
content = r"""<your markdown content here>"""
with open(r'D:\Data\Harold\github\spamfilter-multi\.Codex\memory\current.md', 'w', encoding='utf-8') as f:
    f.write(content)
...
```bash
powershell -NoProfile -Command "Set-Content -Path 'D:\Data\Harold\github\spamfilter-multi\.Codex\memory\memory_metadata.json' -Value '<json content>' -Encoding UTF8"
```

**DO NOT use**:
- The Write tool (blocked for `.Codex/` paths in don't-ask mode)
...
3. **Write the memory file** using Python (see "Writing to .Codex/ paths" above):
   - Use a bash heredoc with Python to write the markdown content to `D:/Data/Harold/github/spamfilter-multi/.Codex/memory/current.md`
   ...
   **Instructions for Codex on Resume**:
   1. Read this context file on startup
   2. Verify git branch matches sprint
   3. Continue from "Next Steps" section above
   ```

4. **Update metadata** using PowerShell at `D:/Data/Harold/github/spamfilter-multi/.Codex/memory/memory_metadata.json`:

   ```bash
   powershell -NoProfile -Command "Set-Content -Path 'D:\Data\Harold\github\spamfilter-multi\.Codex\memory\memory_metadata.json' -Value '{\"current_save\":\".Codex/memory/current.md\",\"last_updated\":\"[ISO timestamp]\",\"sprint\":\"[sprint name]\",\"status\":\"active\",\"pending_restore\":true}' -Encoding UTF8"
   ```
...
- File: D:/Data/Harold/github/spamfilter-multi/.Codex/memory/current.md
