...
2.5. **Sprint Phase 3.7 Approval Verification Gate** (Sprint 36 retro IMP-3)
   ...
   **Procedure**:
   ...
   c. Search for Phase 3.7 approval evidence in three sources, in order:
      ...
      3. **Memory file**: Read .Codex/memory/current.md for any prior-session note recording approval; check .Codex/memory/memory_metadata.json for sprint number match.
...
3. **Memory Restore**: Check if memory should be restored
   - Read D:/Data/Harold/github/spamfilter-multi/.Codex/memory/memory_metadata.json
   ...
   - If pending_restore is true:
     - Read D:/Data/Harold/github/spamfilter-multi/.Codex/memory/current.md and restore context
     ...
     - **Clear pending flag -- MANDATORY, NEVER SKIP**:
       ...
       **Attempt 1 (PREFERRED -- ALWAYS TRY FIRST)** -- PowerShell via Bash:
       bash
       powershell -NoProfile -Command "Set-Content -Path ''D:/Data/Harold/github/spamfilter-multi/.Codex/memory/memory_metadata.json'' -Value ''{\"current_save\":\".Codex/memory/current.md\",\"last_updated\":\"[original timestamp]\",\"sprint\":\"[sprint name]\",\"status\":\"restored\",\"pending_restore\":false}''"
       

       **Attempt 2** -- Bash heredoc fallback (if PowerShell unavailable):
       bash
       cat > "D:/Data/Harold/github/spamfilter-multi/.Codex/memory/memory_metadata.json" << ''EOF''
       {"current_save":".Codex/memory/current.md","last_updated":"[original timestamp]","sprint":"[sprint name]","status":"restored","pending_restore":false}
       EOF
       ...
       **Attempt 4** -- If all are denied by permissions, ASK THE USER:
       ''I need permission to update the memory metadata file to clear the pending_restore flag. Can you approve writing to .Codex/memory/memory_metadata.json?''
