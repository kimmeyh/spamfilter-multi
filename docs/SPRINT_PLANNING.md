...
- **Also applies to manual-validation steps that depend on an external build/environment succeeding at all** (Sprint 57 retro IMP-1), not just to bolting a new capability onto a tool. Before writing "this manual step is not optional" into a plan, confirm the underlying build/environment actually completes right now -- do not assume it will succeed just because the feature under test does not obviously depend on the part that could fail. **Learned from Sprint 57 (F142)**: the plan called the Android-emulator manual validation step "not optional," but the Android debug build itself failed outright at `:app:processDebugGoogleServices` (a pre-existing, unrelated `google-services.json`/`applicationId` mismatch, F94/F150) -- a mismatch F142's own change had nothing to do with. A one-line pre-flight check ("does an Android debug build even complete on this machine right now") would have caught this during planning instead of during execution, and let the plan pre-scope a fallback (as F149's plan did for its own on-demand-reproduction uncertainty) instead of discovering it live.
- **WinWright-specific pre-flight (MANDATORY before ANY WinWright session -- F153, Sprint 59)**: (1) run `mobile-app/scripts/enable-screen-reader-flag.ps1 status` and enable if `False` -- the `SPI_SETSCREENREADER` flag gates ALL Flutter semantics visibility, and skipping this check is exactly how F140 (Sprint 54) recorded a wrong "Flutter exposes nothing" conclusion that stood for 5 sprints; (2) prime the semantics tree with one `ww_get_snapshot` immediately after attach/launch, before any other query (Flutter builds semantics lazily -- the first raw query is empty even with the flag on). Full capability/limitation record: `docs/WINWRIGHT_SELECTORS.md` Prerequisites + the F153 section.
