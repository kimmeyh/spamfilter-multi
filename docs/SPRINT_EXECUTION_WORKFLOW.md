...
- **[WARNING] MANDATORY -- OPEN-THE-FILE VERIFICATION (Sprint 50 retro, Harold 2026-07-27)**: "I believe I did that" is NOT verification, and neither is a summary written from memory of the turn. Before reporting ANY checklist section, phase, or close-out as complete, you MUST:
  1. **Open `docs/SPRINT_CHECKLIST.md`** (Read tool, this turn -- a read from an earlier turn does not count) and walk the relevant section **line by line**.
  2. For EACH line, state its status explicitly: `DONE (<evidence>)`, `N/A (<why>)`, or `NOT DONE -> doing it now`. Evidence means a command output, a file path, or an artifact -- not a recollection.
  3. Only after every line has a status may you report the section complete.

  **The failure this closes (Sprint 50 close-out)**: three Post-Merge items were skipped while the turn reported close-out complete -- five GitHub issues still open (`Closes #N` does not fire on a `feature -> develop` merge), `.claude/sprint_status.json` stale by 15 sprints, and no next-sprint plan stub. None were noticed because the checklist was never opened; the report was assembled from what the turn remembered doing. Enforced by the `verify-closeout-complete` Stop hook, which blocks a completion claim contradicted by the artifacts.

- **[WARNING] MANDATORY -- USER-REQUEST INVENTORY (Sprint 50 retro, Harold 2026-07-27)**: a user message containing multiple asks is a CHECKLIST, not a theme. When a request contains more than one instruction (including a compound sentence, a list, or "do X ... until Y"):
  1. **Enumerate every discrete ask** before starting work -- ideally as harness tasks (TaskCreate), otherwise written out explicitly in your first response.
  2. **Before ending the turn, restate each ask with its status.** An ask that was not done needs either completion or a named `SPRINT_STOPPING_CRITERIA.md` criterion.
  3. A request with a terminal condition ("continue until backlog refinement", "through Phase 7") is NOT satisfied by doing the first part well. The terminal condition is itself an ask.

  **The failure this closes**: Harold asked to "continue following the sprint checklist and SPRINT_EXECUTION_WORKFLOW.md until backlog refinement". The summary/master-plan portion was done and reported as if the whole instruction had been carried out; the Post-Merge section was never reached. Related: Phase 4.1.0 covers approved sprint TASKS -- this rule covers USER ASKS and CHECKLIST ITEMS, which 4.1.0 does not reach.
- **Read the format doc before producing its output (Sprint 45 retro IMP-1)**: when a process step is governed by a named doc that contains an authoritative format/template -- `BACKLOG_REFINEMENT.md` "Backlog Presentation Format", `SPRINT_RETROSPECTIVE.md`'s feedback template, `STORE_RELEASE_PROCESS.md`'s checklists, the ADR template -- READ that doc's format/template section FIRST, then produce the deliverable to match it verbatim. Do NOT generate the format from memory/paraphrase, even if you believe you remember it. Sprint 45 lost a round-trip when a backlog-refinement presentation was paraphrased from memory instead of matching `BACKLOG_REFINEMENT.md`'s spec.
...
2. **Claude Proceeds to Phase 6 (In Parallel) - MANDATORY**
   ...
   **Why the PR update happens during testing**:
   ...
   - The PR reflects current state when testing finishes
...
- [ ] **6.6 Open the NEXT sprint branch immediately after merge** [WARNING] MANDATORY (Sprint 42 retro -- this was missed)
  ...
  - **Why**: once the PR merges, the sprint feature branch is "done." Phase 7 (including retro improvements) completes BEFORE the merge -- so post-merge work is the NEXT sprint's: backlog refinement, GitHub issue cleanup, next-sprint planning, and any follow-up IMPs deferred to Sprint N+1. None of it may be committed onto the merged branch -- those commits get stranded (not on `develop`). They belong on the NEXT sprint branch, created **from the current feature branch** per the Steps below (this paragraph previously said "off the updated `develop`", contradicting its own Steps -- corrected 2026-07-27).
