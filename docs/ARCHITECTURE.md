...
| **ScanCoordinator** (`scan_coordinator.dart`, F175 Sprint 62) | In-process scan mutual exclusion: EVERY scan type acquires a FIFO lease inside `EmailScanner.scanInbox` (released in its `finally`), so manual/background/test/demo scans never run concurrently within ONE ISOLATE; carries the 30-minute `scanTimeout` used by the background-scan timeout wrap and startup stale-`in_progress` reconciliation; on background-scan timeout the hung scan's lease is force-released (owner-matched, `releaseActiveByOwner`) so queued scans proceed instead of waiting out their own limit. **Scope corrected MV74-2 (Sprint 74)**: the lease is per-ISOLATE, so it sees NO background scan on EITHER platform -- Android runs each background scan in its own FlutterEngine/isolate (`workmanager_android` `BackgroundWorker.kt:101`), Windows in a separate Task Scheduler process. Cross-isolate/process DETECTION and EXCLUSION are therefore database-backed and platform-uniform (no exception): the scanning isolate refreshes `scan_results.last_heartbeat_at` every 30s (`heartbeatInterval`); the manual-scan notice counts a background row only with a heartbeat inside `heartbeatFreshness` (5 min); and `BackgroundScanCore.scanAccount` SKIPS an account with a live interactive row (ADR-0039 Sprint 74 amendment): a manual/demo scan's row is written right after its lease and BEFORE it connects (Harold Q1), and re-processing holds a heartbeating `reprocess` claim row (`ScanResultStore.claimInteractive`, deleted when it ends, never listed in Scan History). F109 foreground-deferral still applies on Windows |
...
| **scan_results** | Aggregate scan results per scan | id (PK), account_id (FK), scan_type, scan_mode, started_at, completed_at, total_emails, processed/deleted/moved/safe/no_rule/error counts, status, folders_scanned, last_heartbeat_at (v9, nullable -- cross-isolate liveness, MV74-2) |
...
| **no_rule_review_screen.dart** (F39, Sprint 46) | Cross-account "Review No Rule Items" screen (rule-folder AppBar icon on account selection): aggregates unprocessed `unmatched_emails` rows from each configured account's LATEST completed scan (via `ScanResultStore.getLatestCompletedScan` + `UnmatchedEmailStore.getUnmatchedEmailsByScanFiltered(unprocessedOnly)`), account-filter chips, multi-select (checkbox / Ctrl+click / Shift+click), and 7 bulk actions (safe sender x3, block rule x3, mark reviewed) through the shared `RuleQuickActionService` -- ONE summary notification per bulk operation; handled items marked `processed`. Windows-desktop-scoped in Sprint 46. Note: `ResultsDisplayScreen` also gained the "No rule"-filter auto-advance triage flow (S46-MT-2/3): quick actions pick the next uncovered item FIRST, run the pipeline unawaited, and reopen the popup immediately at the same anchor |
