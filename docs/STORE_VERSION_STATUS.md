...
|---|---|---|---|
| **BUILT, AWAITING UPLOAD** | 0.14.0.0 | 2026-09-04 ~8:55am ET (built + verified by Claude; NOT yet uploaded) | MSIX at `spamfilter-multi-prod/mobile-app/build/windows/x64/runner/Release/my_email_spam_filter.msix`, 17.1 MB. Verified: Check A build-log dart-defines present (`--dart-define=APP_ENV=prod --dart-define-from-file=secrets.prod.json`), `--release-self-test --expected-version=0.14.0` **RESULT: PASS** on all 6 checks (APP_ENV, NATIVE_APP_ENV, displaySuffix, dataDirSuffix, window title, version), manifest Version=0.14.0.0 / correct identity + publisher. **BUILT + VERIFIED IS NOT SUBMITTED** -- this row must not be read as a live version. |
