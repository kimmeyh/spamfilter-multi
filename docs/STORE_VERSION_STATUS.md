...
|---|---|---|---|
| **Live/certified on Store** (cache) | 0.6.2.0 | 2026-08-13 (Harold, Partner Center screenshot: Application overview banner "Congrats! Your product is now updated", Store presence card "Submission 13: Last modified on 08/13/2026" with green checkmark confirming current availability) | Submission 13, certified and published same day as submission (F148 -- background-scan Store-update survival). Superseded 0.6.1.0/Submission 12. **Known gap**: this build used a local-only, unpushed develop->main merge -- `main` on GitHub does not yet contain F148; Harold still needs to do the real merge so history matches what shipped. See `.claude/sprint_status.json` `store_release` block for full detail. |
| **Dev worktree** (authoritative -- mirrors `pubspec.yaml`) | 0.6.2+1 | 2026-08-13 | Bumped for the 0.6.2.0 release build (F148) per `docs/STORE_RELEASE_PROCESS.md` Step 1 -- PATCH, not MINOR, since `[Unreleased]` contains only `fix` entries. `msix_version` bumped to `0.6.2.0` in the PROD worktree (local/uncommitted, per convention) for this in-flight build. |
