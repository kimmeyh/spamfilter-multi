...
|---|---|---|---|
| **Live/certified on Store** (cache) | 0.6.1.0 | 2026-08-13 (Harold, confirmed directly in chat that Submission 12 is certified and live -- formal Partner Center screenshot evidence not yet attached; see `.claude/sprint_status.json` `store_release._live_version_note`) | Submission 12, certified and published. Superseded 0.6.0.0/Submission 11. **0.6.2.0 (PATCH-only, F148) SUBMITTED for certification 2026-08-13 -- NOT yet live, do not update this row until Partner Center confirms certification.** Known gap: this build used a local-only, unpushed develop->main merge -- `main` on GitHub does not yet contain F148; Harold still needs to do the real merge. See `.claude/sprint_status.json` `store_release` block for full detail. |
