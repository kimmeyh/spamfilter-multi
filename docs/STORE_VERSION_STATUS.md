...
|---|---|---|---|
| **SUBMITTED, IN CERTIFICATION** | 0.14.0.0 (listing-only) | 2026-09-07 (Harold submitted; Pre-processing observed) | Submission 22. **NO new package** -- metadata only: the Description's privacy paragraph rewritten so the local-only promise leads and the 100-character preview is described as saved ON THE DEVICE; privacy policy URL set to the canonical GP-5 address; and the OneDrive automatic-backup product declaration UNCHECKED so the description needs no asterisk. **The LIVE version is unchanged at 0.14.0.0 / Submission 21** -- this alters listing text, not the app. |
| **Live/certified on Store** (cache) | 0.14.0.0 | 2026-09-05 ~10:32pm ET (direct Partner Center observation: "Congrats! Your product is now updated", Store presence = Submission 21, last modified 09/05/2026 -- PLUS an INSTALLED-BUILD CHECK: the running Store app's header reads Version 0.14.0 with no [DEV] marker, the strongest verification class) | Submission 21. Eighth MINOR release (Sprint 64 feat: F186 Body Phrase rule type). Supersedes 0.13.0.0 / Submission 20. Certification elapsed NOT measurable this cycle -- both the upload and the observation are ranges, so the true time is unknown rather than slow. |
