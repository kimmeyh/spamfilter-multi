...
half changes Windows behavior, so it is now described below.
...
- **Backgrounding a live scan -- the Windows-relevant half is the manual-scan timeout above.** The
  original trigger is Android-only: Windows does not tear down sockets when a window is minimized.
...
- **Toolchain upgrade -- still excluded.** Developer tooling. Its acceptance criteria specifically
  checked for behavior changes and found none a user would see.
