...
|---|---|---|---|---|
| 22 | 0.14.0.0 (listing-only; NO new package) | 2026-09-07, observed in Pre-processing (Harold's screenshot; Partner Center shows Submission 22 last modified 09/07/2026) | (pending) | (pending) -- metadata-only submissions are usually faster than a package release, but this is the first one measured, so there is no band to compare against. |
