...
- [ ] **5.1.5 WinWright UI Test Sweep** (MANDATORY -- Sprint 38 retro): update WinWright scripts for sprint UI changes; run all scripts that exercise sprint-touched screens; fix in-sprint or backlog (see SPRINT_EXECUTION_WORKFLOW.md § 5.1.5). If no WinWright coverage exists for sprint's UI surface, file a Sprint N+1 carry-in. **ARTIFACT REQUIRED (Sprint 59 retro IMP-5; extended Sprint 62 retro IMP-2)**: record the sweep's date, script count, pass/fail summary, AND a `sweep-head: <hash>` line naming the commit the sweep ran at in the sprint plan's completion notes -- a checked box with no recorded evidence is how this step silently went unrun from Sprint 52 through Sprint 58, and a sweep without its hash is how Sprint 61 shipped F169 with every script selector rotten (the close-out hook verifies no lib/ui commit is newer than sweep-head).
- [ ] **Phase 5 evidence gate (Sprint 62 retro IMP-1)**: 5.1.1 + 5.1.2 + 5.1.5 evidence must EXIST (review record, F-PRECHECK on the PR, sweep artifact in the plan) BEFORE Phase 5.3 Manual Validation is declared started -- Sprint 62 reached MV with all three silently unrun, and the late review then found a real bug (C-2) after Harold had already validated. Enforced from Sprint 63 by `verify-closeout-complete` checks 3d/3e at any close-out claim, **and from Sprint 67 (F193) by `sprint-auto-advance` Gate 1c AT THE MANUAL-VALIDATION BOUNDARY** -- the moment `.claude/sprint_status.json` `current_sprint.status` first names Manual Validation / Phase 5.3+, the three markers must be RECORDED in `SPRINT_N_PLAN.md`, not left as `PENDING`. **Why both**: the close-out check fires far too late. Sprint 66 declared MV, Harold validated on real devices, the sprint closed out, the retrospective was written and four improvements were applied -- and only THEN did the close-out hook report that 5.1.1 had never run. The review, once run, found three policy gates that passed while catching nothing (one completely inert). Sprint 62 was the same shape and its late review found a real user-affecting bug. A marker reading `PENDING`/`TBD`/`TODO` does not count -- that placeholder exists to say NOT DONE, and the plan template seeds it. `N/A` counts only **with a reason attached** (`N/A -- no lib/ui change this sprint`); a bare `N/A` is a shrug, not a decision.
