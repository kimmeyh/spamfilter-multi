...
- **5.1.5 WinWright sweep**: 2026-09-25 RE-RUN at `a3e0420` after the MV Q1 change touched `lib/ui` (re-process claim) -- **2 of 2 PASSED, 29/29 steps each, no DB drift.** The first attempt at `a3e0420` FAILED `test_mt2c_no_rule_sweep` (step 6: the Gmail account chip's bounds 'kept changing -- animating'; retry step 3: the AOL chip resolved 0 elements). Data hypothesis (Clear history removing an account's chip) FALSIFIED against the dev DB (AOL 4,397 no-rule items intact). Cause NOT established; consistent with the documented F226 live-input interference -- Harold was validating at the time. Earlier sweep: 2026-09-25 at `92411c5` (dev exe rebuilt 09:45 from that commit; it contains every `lib/ui` change of the sprint) -- **2 of 2 scripts PASSED, 29/29 steps each, no DB drift**, workstation-unlocked check passed. Scripts: `test_f124_rule_labels.json` (17s), `test_mt2c_no_rule_sweep.json` (21s). New UI with NO script yet -> Sprint 75 carry-in (5.1.5 step 5): Scan History Clear history, Settings "Hide sender details in exports", the Account tab's resolved-default folder rows.
