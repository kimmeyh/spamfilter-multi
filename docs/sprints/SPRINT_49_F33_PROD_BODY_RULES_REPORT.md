...
**Total body-condition rules**: 2226

## Group counts

| Group | Meaning | Action | Count |
|-------|---------|--------|-------|
| G1 | ALL domain-shaped w/ full `.tld` (Option B) | Convert to URL-anchored regex | 1300 |
| G2 | keyword/phrase misclassified as domain | Reclassify metadata (body unchanged) | 168 |
| G4 | adamshetzner without `.tld` | Remove | 4 |
| G5 | orphan / empty condition_body | Remove | 0 |
| G6 | truncated/bare `/label\.` or `/label`, no full tld | Remove | 742 |
| DUP | same-domain-root duplicates | Remove all but first | 2 |
| SPECIAL | Harold hand-decided (phone-number rewrite + 2 removals) | Convert 2 / Remove 4 | 6 |
| ? | ambiguous (does not fit) | Report only, untouched | 4 |

**Net removals**: 752. **Reclassified**: 168. **Converted**: 1302 (G1 1300 + SPECIAL 2).

## SPECIAL (Harold hand-decided 2026-07-07)

- CONVERT id=2511: `["800\\-571\\-7438"]` -> `["\(?800\)?[-. ]?571[-. ]?7438"]`
- CONVERT id=6226: `["800\\-571\\-7438"]` -> `["\(?800\)?[-. ]?571[-. ]?7438"]`
- REMOVE `body_.nl` body=`["\\.nl/"]`
- REMOVE `body_sys-confg.co.ukcl` body=`["sys\\-confg\\.co\\.uk/cl/"]`
- REMOVE `\.nl/` body=`["\\.nl/"]`
- REMOVE `sys\-confg\.co\.uk/cl/` body=`["sys\\-confg\\.co\\.uk/cl/"]`
...
- id=2172: `["/amigrpinc\\.com"]` -> `["(?:://|[/.])amigrpinc\.com"]`
- ... and 1285 more

## G2 keyword reclassifications (168)
...
- `/adamshetzner\.` body=`["/adamshetzner\\."]` subType=`entire_domain` src=`/adamshetzner\.`
- `\.adamshetzner\.` body=`["\\.adamshetzner\\."]` subType=`entire_domain` src=`\.adamshetzner\.`
...
## G6 truncated/bare removals (742)
...
## Ambiguous (untouched -- review) (4)

- `/%\.nl/` body=`["/%\\.nl/"]` subType=`entire_domain` src=`/%\.nl/`
- `/%\.ru/` body=`["/%\\.ru/"]` subType=`entire_domain` src=`/%\.ru/`
- `/%\.store/` body=`["/%\\.store/"]` subType=`entire_domain` src=`/%\.store/`
- `/.*\.xyz` body=`["/.*\\.xyz"]` subType=`entire_domain` src=`/.*\.xyz`
