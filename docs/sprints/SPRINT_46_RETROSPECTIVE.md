...
- **Copilot round-6 carry-ins (Sprint 47)**: after 5 fix rounds (21 comments addressed), two low-severity round-6 comments were dispositioned to Sprint 47 rather than looping further: (a) `cleanup_body_rules.dart` -- a JSON-decode failure classifies the row as a G5 orphan (deletable on `--apply`) instead of reporting it as ambiguous; MUST be fixed before the already-planned Sprint 47 `--env prod --apply` run (dev-run evidence: all 1109 rows decoded cleanly, so no data was affected). (b) `no_rule_review_screen.dart` load-error handling: add stackTrace to the log and replace the raw exception in the SnackBar with a friendly message -- cosmetic polish.
