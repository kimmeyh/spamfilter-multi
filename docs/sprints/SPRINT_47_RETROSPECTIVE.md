...
- **IMP-3 (Proposal 3) -- DONE** (after Harold approved the `.claude/` write): the stash-guard PreToolUse hook `.claude/hooks/block-carry-forward-stash.ps1` is authored AND wired into `.claude/settings.json` (a `PreToolUse` entry, matcher `Bash|PowerShell`). Verified live in the harness: `git stash` is blocked (exit 2 + correction), `git status` / `git stash list` / a command carrying the `allow_stash` bypass token all pass (exit 0). The block was even proven by the hook firing on the verification command itself. The `.claude/`-write restriction under don't-ask mode is why this initially blocked; Harold's one-time approval unblocked it.
