...
**COMPLETION NOTES (2026-08-18)**: CODE COMPLETE; on-device verification (AC-2) scheduled into Manual Validation. Delivered as the ADR-0042 platform factory Harold endorsed:

- **`background_scan_core.dart`** -- the per-account scan orchestration extracted VERBATIM from the Windows worker's `_scanAccount` (settings resolution with `isBackground: true`, headless provider, shared `EmailScanner`, `scanType: 'background'`), so both platforms run the IDENTICAL pipeline and persist identically. Duplicating it into the Android worker was rejected explicitly: a platform-local copy is where the Sprint 60 accounts-FK class breeds. The Windows worker now delegates through a thin wrapper; its call sites, `_ScanResult` type, and the scheduler service are untouched (AC-4).
- **`background_scan_scheduler.dart`** -- `BackgroundScanScheduler` (small shared contract: isSupported/isScheduled/schedule/cancel), `WindowsSchedulerAdapter` (the exists->update/create decision settings_screen used to make inline), `AndroidSchedulerAdapter` (WorkManager per-account unique work: name mirrors the Windows `SpamFilterBackgroundScan_<sanitized><envSuffix>` convention with the same sanitizer and env suffix; `NetworkType.connected`; `ExistingPeriodicWorkPolicy.update` for idempotent rescheduling), `UnsupportedPlatformScheduler` (the ADR's explicitly-NAMED no-op -- schedule returns false, never a silent success), and the factory with a test override seam. Windows-only capabilities (task-path repair, orphan enumeration) deliberately stay OFF the interface per the ADR's shared-contract rule.
- **`android_background_scan_worker.dart`** -- the WorkManager dispatcher (`@pragma('vm:entry-point')`, own binding + `DartPluginRegistrant` for the background isolate) and worker mirroring the Windows preamble (paths, DB, stores, rules; enabled-check skipped for test runs), plus the completion notification -- the R-2 call site. AC-3 honored: POST_NOTIFICATIONS is requested contextually in Settings when the user enables background scanning or runs a test, and the manifest declaration carries the F161 comment.
- **Wiring**: `main.dart` registers the dispatcher (Android-gated, best-effort); `settings_screen`'s `_updateWindowsScheduledTask` renamed `_updateScheduledScan` and rerouted through the factory -- the call site is now platform-free, and the Test button runs the real one-off WorkManager path on Android instead of showing "Windows only".
- **Tests (9, all passing)**: the Android adapter is tested REAL against a fake `WorkmanagerPlatform` via the plugin's own injection seam -- asserting the exact registration payload (unique name incl. env suffix, frequency, accountId inputData, network constraint, UPDATE policy), disabled-frequency refusal, platform-rejection -> false, cancel targeting, isScheduled round-trip, and the one-off test task. The UPDATE-policy assertion is mutation-verified (flipping to `keep` -- the stale-frequency trap the plugin docs name -- turns it red). The Unsupported no-op contract and factory override are pinned. `WindowsSchedulerAdapter` is deliberately NOT unit-tested, with the reason in the test's coverage map: its three calls shell out to schtasks.exe, so a unit test would either run real scheduler commands on the host or mock itself; the live path runs on the dev machine daily.
- **API verified against installed source, not memory**: workmanager resolved to 0.10.7 (newer than the 0.9.0+3 F144 removed); `registerPeriodicTask`/`isScheduledByUniqueName`/`ExistingPeriodicWorkPolicy`/`Constraints` signatures read from the pub cache before use.
- **Declared platform exceptions (AC-5)**, in the adapter doc: WorkManager's 15-minute floor (defensive -- `every15min` is already the app minimum), inexact battery-batched timing versus Task Scheduler's exact timing, and the network-required constraint.
- **Known limitations, stated rather than hidden**: (a) AC-2's on-device persistence verification is a Manual Validation step -- Harold enables background scanning / taps Test on the emulator, then the DB is pulled via `adb run-as` + sqlite3 to confirm scan_results/email_actions/unmatched_emails rows (the method that caught both Sprint 60 bugs); the APK with this code is built and installed for exactly that. (b) No Android orphan-schedule cleanup at startup yet (Windows prunes tasks for removed accounts; the Android worker handles a missing account gracefully and returns non-retryable success) -- follow-up candidate, not silent. (c) The settings call-site reroute has no widget test (would need the full account+DB harness); the reroute is mechanical and the end-to-end path is what MV verifies.
...
**AC-2 SATISFIED ON-DEVICE (2026-08-18 evening, with Harold driving)**: after the gate fix, the full chain ran live -- enable toggle -> POST_NOTIFICATIONS prompt appeared (Harold: "permission dialog accepted") -> WorkManager job registered (confirmed in dumpsys jobscheduler) -> Test button queued the one-off -> dispatcher executed -> `Worker result SUCCESS`. **DB pulled via `adb run-as` + sqlite3, rows verified**: `scan_results` id 6 (`kimmeyh@gmail.com | background | readOnly | completed | 3 found / 1 processed / 1 no-rule`), 1 `email_actions` row, 1 `unmatched_emails` row (noreply@qbraid.com, INBOX, unprocessed), all `accounts` rows present (the Sprint 60 FK fix holding). **Better still: scan id 5 is a SECOND background scan -- the scheduled periodic path fired on its own**, so both the Test path and the real 15-minute schedule are proven on-device (AC-1 and AC-2 both satisfied by observation).

**Residual finding for the backlog**: during the scan, IMAP fetch from the "Unwanted" folder threw `InvalidArgumentException: no ID added to sequence` -- the scan completed and reported errors=0, so a folder silently failed to fetch WITHOUT surfacing in the error count. Pre-existing scanner edge case (likely an empty-folder sequence-set bug), not F161 -- but a per-folder fetch failure that leaves no trace in the results is a small cousin of the F168 silent-scope class and deserves an item.
