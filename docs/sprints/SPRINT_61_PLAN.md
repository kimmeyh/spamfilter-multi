...
- R-2: Canonical order: sprint PR merged to `develop` -> `develop` merged to `main` (Harold) -> **Backlog Refinement pass 1 = COMPLETENESS SWEEP** (verify every sprint-close step was captured and completed) -> **Microsoft Store release** -> once the submission is in process, **Backlog Refinement pass 2 = SCOPE SELECTION** -> Phase 3 planning.
...
- R-7: `STORE_RELEASE_PROCESS.md` states WHERE in the cycle a release sits, and its internal ordering conflict is reconciled deliberately: its Step 5 puts the `main` merge AFTER the version bump and build, while the cycle has it at step 2. Pick one, state why, make both docs agree.
...
1. **Task 1 R-3** (Class 3): correcting the "Canonical Next Steps progression", currently flagged MUST NOT be reordered. Approve the correction?
2. **Task 1 R-7** (Class 3): `STORE_RELEASE_PROCESS.md` puts the `main` merge AFTER the version bump and build; the cycle puts it at step 2. Which ordering is authoritative?
3. **Task 6** (Class 1): the parity ADR will be presented for approval before Tasks 7 and 8 treat it as binding.
4. **Sprint size**: 9 tasks at ~17-27 hours is larger than recent sprints. Confirm, or drop the largest (Task 8, F161) to a following sprint.
