...
**Sprint 45 candidate pool** (active backlog below): the near-term active items shipped in Sprints 43-44. Remaining candidates are **template deep dives (F70 Security / F71 Architecture, run periodically)**, **F64 (CI/CD, HOLD)**, **Post-MVP** items (SEC-11b DB encryption + F106 cleanup), and the **HOLD platform/UX tracks** (F94/F95 flavors, F63 responsive, SEC-8b/SEC-15, F6, H1-H5). Harold selects Sprint 45 scope in Phase 1.2. **Note**: an Android-device retest of the Sprint 44 F108 dep bumps is an open post-merge verification item (see Last Completed Sprint).
...
_(F107, F108, F109 shipped in Sprint 44 -- see docs/sprints/SPRINT_44_SUMMARY.md.)_
