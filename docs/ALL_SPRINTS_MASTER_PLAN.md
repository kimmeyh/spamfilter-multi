...
**F160. Skipped-tests audit -- [DONE Sprint 60, 2026-08-16: 29-test decision table delivered; Harold approved ALL recommendations; 3 discontinues deleted (29 -> 26 skips), 11 update-to-working verdicts registered as F163, 15 keeps unchanged. See SPRINT_60_PLAN.md Task 1 + MV round 1.]**
...
**F156. Full Android app testing walk-through -- [DONE Sprint 60, 2026-08-16: walk-through complete (32/32 on-device integration); found+fixed 2 CRITICAL Android bugs (value-returning PRAGMAs rejected by execSQL -> every scan failed; accounts-FK gap -> zero scan persistence) plus the MV-round fixes. See SPRINT_60_PLAN.md Task 7 + MV rounds 1-4.]**
...
- Scope (Harold, 2026-08-15): impact study of the migrator's suggested change (and any related gradle/SDK modernization it implies): what does `flutter.minSdkVersion` resolve to on the current toolchain, does it satisfy the F108 >= 23 requirement (adopt as-is if yes; if it resolves below 23, evaluate expressing the floor Flutter's way), what else does the migrator want to change, and what breaks. **Include implementation in this item if the measured impact is < 200 lines of code AND < 2 hours wall-clock**; otherwise report findings and split implementation out.
...
**F158. CI guard for the F150 regression class -- [DONE Sprint 60, 2026-08-16: android-build CI job added (temurin 17, committed CI-safe google-services stub with fake values, `flutter build apk --debug`); design question resolved as validate-with-stub. See SPRINT_60_PLAN.md Task 3.]**
...
**F159. Metadata-under-gates -- [DONE Sprint 60, 2026-08-16: msix_version == pubspec X.Y.Z + '.0' asserted in both the policy test and check-version-consistency.ps1; dependency-floor comment checks evaluated and the deliberately-unwatched remainder recorded. See SPRINT_60_PLAN.md Task 4.]**
...
**F166. Scan Results header redesign -- [DONE Sprint 60, 2026-08-16: executed in-sprint per Harold's plan-then-execute steering (card #336); dropdown filter with No Rule default + Folders chip on one line, inline scan-complete, simplified no-rule banner, compact-width popup. See SPRINT_60_PLAN.md MV round 3.]**
...
- Leading hypothesis (code-read at registration, NOT yet confirmed): `SettingsStore.getEffectiveFolders(accountId, isBackground: true)` resolves a background-SPECIFIC folder override (`getAccountBackgroundScanFolders`) BEFORE the generic per-account override, so a background override of {Bulk, Bulk Mail} silently wins over the manual selection that includes Inbox. Two separate, independently-editable folder scopes for the same account is the suspected design flaw -- a user who selects folders in the obvious place reasonably expects background scans to honor them.
- Scope: confirm the resolution path against Harold's actual stored settings (read `app_settings` in the PROD data directory -- do NOT write to it); decide whether the two scopes should remain independent (then surface the background scope clearly in Settings, including which folders the next background run will use) or whether background should default to the manual selection when no explicit background override was set; fix accordingly with a regression test pinning the resolution order.
- **Note on Sprint 60**: the two Sprint 60 Android fixes (PRAGMA, accounts-FK) are unrelated to this -- both were Android-only failures, and this is Windows production. Harold flagged "possible it was fixed this sprint" -- it was not; nothing in Sprint 60 touched folder-scope resolution.
- **CONFIRMED BY HAROLD (2026-08-16, same session): NOT A SCANNER BUG.** Harold: *"looks like I did not select inbox as one of the 3 folders. (but thought I had)."* The background folder selection genuinely did not include Inbox, so the scan behaved exactly as configured. Harold clarifies the affected configuration is the **PRODUCTION app's background/bulk scan** folder scope specifically (the dev build and the manual-scan scope are separate selections and were not the ones missing Inbox) -- which matters for the fix: prod and dev keep independent settings stores (ADR-0035), so the UX work must be verified against the PRODUCTION configuration surface, and any repro must read prod settings rather than dev. That REFRAMES this item from a defect to a **UX/discoverability** item, which is arguably the more valuable finding: the app let a user believe Inbox was covered when it was not, and the failure was silent -- a background scan reporting "Found 0" looks identical whether there is no spam or the folder scope is wrong.
...
- Problem (Harold, 2026-08-16): "This will be a common request on almost every PR" -- the real repeating cycle is (1) sprint PR merged to `develop`, (2) `develop` -> `main` merge, (3) **Backlog Refinement pass 1 = a COMPLETENESS SWEEP** confirming every sprint-close step was captured and completed, (4) the **Microsoft Store release**, (5) once the store app is in process, **RETURN to Backlog Refinement pass 2 = SCOPE SELECTION**, (6) Phase 3 planning. Two refinement passes with DIFFERENT purposes, with the Store release between them, and next-sprint scope NOT selected until the submission is underway. **No document encodes this sequence.**
- Evidence the gap is already biting: the ONLY artifact in the repo describing Harold's ordering is `.claude/sprint_status.json` -- a STATE file carrying hand-written prose (`_pr_number_null_reason`, `next_actions`) doing a process doc's job. The `verify-closeout-complete` hook fired as a FALSE POSITIVE twice in one session (2026-08-16) because a null `pr_number` is legitimate for the whole window between merge and scope selection.
- **The one hard code defect**: `.claude/hooks/verify-closeout-complete.ps1` (the `pr_number` check) has NO precondition on `plan_approved`. Its own siblings demonstrate the fix pattern -- the open-issues check is gated on PR-merged state with the comment *"the check tested a post-condition without testing its PRECONDITION. A hook that fires on correct work trains people to bypass it, which costs more than the escape it prevents"* -- and `require-sprint-cards.ps1` gates on `plan_approved` for exactly this reason. This check is the one sibling that never got the treatment.
- **The one authoritative ordering statement that is WRONG**: `SPRINT_EXECUTION_WORKFLOW.md` "Canonical Next Steps progression" (flagged MUST NOT be reordered) rule 5 reads *"Sprint N+1 begins only AFTER merge to develop"* -- it jumps from the develop merge straight to Sprint N+1, with no `main` merge, no Store release, and no refinement passes.
- Known scope (from the Sprint 61 survey): `SPRINT_EXECUTION_WORKFLOW.md` (Phase Numbering table, Phase 7.9's 3-option Next Steps menu, the canonical progression); `SPRINT_CHECKLIST.md` (its Store section is marked "if applicable" and buries `Merge develop to main` as a Store sub-step; no refinement step appears after merge at all; its abbreviated build command has drifted from the authoritative process and omits the mandatory F119 verification); `BACKLOG_REFINEMENT.md` (single-pass model, and its "on-demand / when requested by the Product Owner" trigger CONTRADICTS the Sprint 36 mandatory-every-sprint policy -- fix while editing or the two-pass encoding inherits the contradiction); `STORE_RELEASE_PROCESS.md` (never says where in the cycle a release sits; its internal step order puts the `main` merge after the version bump, conflicting with the cycle's ordering -- reconcile deliberately); `ALL_SPRINTS_MASTER_PLAN.md` Maintenance Guide (one undifferentiated "Backlog Refinement" row for two passes; no Store-release row despite the release mutating this file); `CLAUDE.md` (branch policy says WHO/HOW but never WHEN; the Auto-Advance window is defined by two in-sprint endpoints and never names this outside-the-window region); skills `backlog-refinement` (hardcodes one pass whose step 5 always captures a scope selection -- wrong for pass 1), `startup-check` (no phase state for "in release cycle"), `phase-check` (phase model is 1-7 only).
- Also: `sprint-auto-advance.ps1` is safe during the cycle only by ACCIDENT (no plan file yet). It becomes fragile the moment a next-sprint plan stub exists -- which the checklist itself mandates at Phase 7.7 -- so the exemption needs to be deliberate rather than incidental.
- Deliverables: a NAMED cycle (avoid "Step 7", which already means two different things in two authoritative docs) documented once and cross-referenced everywhere; the hook precondition fixed with a test case covering the post-merge/pre-scope state (no existing case covers it); the two-pass distinction encoded in the refinement doc and skill; the mandatory-vs-on-demand contradiction resolved; doc version footers refreshed (all four are stale).
- Source: Harold, 2026-08-16, after the cycle ran manually end-to-end for Sprint 60 -> 0.10.0.0 and the hook misfired twice.

**F169. Review No Rule Items account filter: replace the scrolling chip row with a single-select dropdown, default All Accounts (~1-2h incl. tests) Priority 4 (NEW, Sprint 61 -- Harold; ASSIGNED TO SPRINT 61)**
- Phase: Core App Quality / UX
- Platform: All (shared widget -- Harold explicitly approved the same treatment on BOTH Windows and Android)
- Observation (Harold, 2026-08-16, Android emulator screenshot): with two accounts configured, the account filter renders "All Accounts (17)" and "kimmeyh@gmail.com (0)" as chips in a horizontally scrollable Row (`no_rule_review_screen._buildAccountFilter`, line ~793). On a phone the second chip is clipped at the right edge and the AOL account chip is off-screen entirely, with NO affordance indicating the row scrolls -- so an account is effectively unreachable. **"All account must be viewable."**
- Direction (Harold): "would a drop-down similar to what we just did for the Scan Results (No Rule, Safe...) work here with default to All" -- yes: F166 (Sprint 60) established exactly this pattern on the Results screen (a `PopupMenuButton` chip whose face shows the active selection with its live count, entries carrying per-option counts, single-select by construction). Reuse that pattern rather than inventing a second one.
- Scope: replace the chip Row with one dropdown chip; default face "All Accounts (<total>)"; one entry per configured account with its item count; selecting an entry sets `_accountFilter` and re-applies exactly as the chips do today (including `_clearSelection()` on change, which the current handler does and must be preserved). Width-independent by construction -- no horizontal scroll, no clipping at any width.
- Test expectations: a widget test at phone width (< 600px) asserting EVERY configured account is reachable from the dropdown (the current failure mode: an account rendered off-viewport); default-selection assertion (All Accounts); and a selection-clearing assertion on account change. Mutation-verify each.
- Consider while implementing: F166's dropdown entries carry a plain-language explanation line (the restored F151c texts) -- account entries likely need only the email + count, but check the F166 builder before diverging.
...
- **SHIPMENT PREREQUISITE (Harold, 2026-08-18)**: "we have not delivered to customers an Android package yet. The scheduler will need to be in place (key requirement) before shipment." F161 therefore GATES the Google Play track (F94, GP-*) -- Android ships with background scanning or it does not ship. Recorded in ADR-0042 under deliberate non-parity, with the consequence that user-facing content must NOT carry "not available on Android yet" caveats for this gap.
