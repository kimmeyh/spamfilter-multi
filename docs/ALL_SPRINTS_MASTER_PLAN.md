...
- **Store status**: The defective `0.5.4` (F119: `msix_config` typo `build_windows_args` vs the correct `windows_build_args` -> MSIX ran as `APP_ENV=dev` with empty Gmail OAuth credentials) went live 2026-07-15. **The corrected `0.5.5.0` MSIX was built from the fixed prod worktree, verified (Step 4.0: build log confirms `--dart-define=APP_ENV=prod`; manifest `0.5.5.0`; 17.6 MB), and SUBMITTED to Partner Center for certification on 2026-07-19** -- now IN CERTIFICATION (typical 24-72h). This is the first true public release (2 -> ~20 users). On cert PASS: run the STORE_RELEASE_PROCESS Step 7 close-out (master-plan Last-Completed-Sprint update, CHANGELOG `[0.5.5]` heading, dev bump 0.5.5 -> 0.5.6) and the Android/Google Play track comes OFF HOLD.
