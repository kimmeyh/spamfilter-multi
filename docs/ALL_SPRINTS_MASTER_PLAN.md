...
**F153. Re-investigate Flutter Windows UIA/accessibility-tree exposure to WinWright -- MAJOR LEAD FOUND, re-test needed (~time-boxed, no-history) Priority 10 (upgraded from HOLD)**
...
- **What this covers (do this)**:
  ...
  5. Verify the installed release candidate the same way as any Store build: `<InstallLocation>\MyEmailSpamFilter.exe --print-env` (launch via `Start-Process ... -RedirectStandardOutput` to reliably capture output; a bare `&` invocation from a script can silently produce nothing) should show `APP_ENV=prod`, empty `displaySuffix`/`dataDirSuffix`, `windowTitle=MyEmailSpamFilter`, `NATIVE_APP_ENV=prod`. Launch the actual window via its AppX identity (`Start-Process "shell:AppsFolder\<AppID>"`, from `Get-StartApps`) for the visual title-bar/About-screen check, not the raw exe path directly (packaged apps are normally activated through their app identity). **Known gap, STATUS UNCERTAIN as of 2026-08-15 (see F153)**: WinWright was believed unable to scroll to the About screen's version text (Settings > General, end of tab) via automation, per F140's Sprint 54 finding. F153's Sprint 58 discovery (F140's negative result may have been caused by a missing `SPI_SETSCREENREADER` OS flag, not a genuine Flutter limitation) means this specific gap needs re-verification with the flag correctly enabled before being treated as still true. Until F153's re-test confirms one way or the other, continue treating the visual About-screen confirmation as requiring a Harold-performed check (the `--print-env` probe and title-bar check remain independently reliable regardless).
