...
**F151. Windows first-run experience -- welcome/orientation, Demo Mode discoverability, walkthrough callout, scan-results clarity, and human-readable connection/scan errors (~2-3h) Priority 10 (Windows only -- Android deferred to Android rollout track)**
- Phase: Core App Quality / UX
- Platform: Windows Desktop only (explicit Harold steering, 2026-08-15: apply for Windows now, adapt Android later during the Android rollout track alongside F142/F143/F144/F150)
- **Source**: Harold provided a user-centric MVP test script (install -> launch -> understand purpose -> complete primary task -> clear feedback) plus a detailed "First-Run Evaluation Summary Page" (7 sections: Installation & Launch, First-Run Orientation, Completing the Primary Task, Feedback & Confirmation, Emotional Experience, Environment Compatibility, Quick User Feedback Capture) as Sprint 58 Backlog Refinement input, 2026-08-15. Investigated via an Explore agent mapping the actual first-run code paths, then CONFIRMED via a real live Windows walkthrough (built+launched the actual 0.7.0.0 prod exe with a genuinely empty account/data state -- see Investigation Note below) with Harold driving the app and Claude observing/screenshotting via WinWright.
- **Finding**: the core workflow (add account -> OAuth/IMAP credentials -> scan -> results -> tune rules) is fully built and functional. The gap is entirely about a first-time user DISCOVERING that workflow, UNDERSTANDING its results, and TRUSTING its outcomes, not missing functionality. Seven separable, code-verified and/or live-walkthrough-confirmed sub-gaps, one investigation-only sub-item:

  1. **No welcome/orientation screen.** Live-confirmed: a genuinely brand-new user (zero accounts, verified via a real prod-exe launch with all account data cleared) sees `AccountSelectionScreen`'s empty state (`account_selection_screen.dart:727-736`, `empty_state.dart:83-101`) -- an icon, "No Accounts Yet," "Add your first email account to start scanning for spam.," and a single "Add Account" button. Nothing explains what the app does, what a "rule" is, or that a safe (read-only) way to explore exists.
  2. **Demo Mode is buried.** Live-confirmed: only surfaced one tap deep, inside "Add Account" -> `PlatformSelectionScreen` (`platform_selection_screen.dart:122-132`) -- not visible from the first screen a new user actually sees, though well-presented once reached (prominent top card, "Test with 50+ sample emails (no email account needed)").
  3. **The built 8-step walkthrough is opt-in and buried.** `mobile-app/assets/content/help/walkthrough.md` is current and substantial (rewritten Sprint 55) but sits as the LAST of 22 sections on `HelpScreen` (`help_screen.dart:258-259`), reachable only via a manually-tapped `?` icon. F75 (existing HOLD item, below) explicitly scoped OUT both a "First time? Start here" Help-screen callout and a first-launch onboarding overlay ("out of scope for v1; flag for future consideration") -- no successor item currently owns finishing either piece.
  4. **Scan Results screen is information-dense with no first-time explanation** (live-confirmed via a real Demo Scan run). 7 colored summary chips (Found/Processed/Deleted/Moved/Safe/No rule/Errors), a filter banner, a "no-rule" progress bar, and a sender-grouping callout all appear at once with no tooltips or first-time context. Specific findings from the live run (Harold, 2026-08-15):
     - Add tooltips to all 7 colored summary chips, on the Demo Scan results screen AND all real Scan Results screens (same shared widget).
     - The built-in demo dataset (50+ sample emails) has NO safe-sender example -- add at least one so a first-time user's very first scan shows all the major outcome categories, not just delete/no-rule.
     - The **"Moved" chip is dead** -- the underlying move-on-match feature is not yet implemented, so it always shows `0` alongside real, populated chips (Found: 59, Processed: 38, Deleted: 26, No rule: 12), which reads as a bug/confusing rather than an intentional zero. Remove the "Moved" chip from the Demo Scan screen AND all real Scan Results screens until the feature ships.
     - "Deleted (not processed): 26" is ambiguous wording -- "Deleted" and "(not processed)" read as contradictory to someone who doesn't already know this is Read-Only-mode language (nothing was actually deleted; it reports what WOULD be deleted). Candidate for either a tooltip (folds into the chip-tooltip item above) or a wording pass.
  5. **Email-detail action popup is oversized at the default window size** (live-confirmed, screenshot from Harold, 2026-08-15). The "Add to Safe Senders" (3 buttons: Exact Email / Exact Domain / Entire Domain) and "Create Block Rule" (3 buttons: Block Email / Block Exact Domain / Block Entire Domain, plus a partially-cut-off "Block Subject" row) sections stack full-width vertically below their section labels, overflowing the default screen size so the popup must scroll before a user can see all options. Layout fix: move each set of 3 buttons to sit beside its section label (not stacked below) and shrink them, so the popup fits without scrolling at default window size.
  6. **Two connection/scan error paths show raw exception text instead of the existing friendlier widgets.** `ErrorDisplay`/`AuthenticationErrorDisplay`/`NetworkErrorDisplay` (`mobile-app/lib/ui/widgets/error_display.dart`) already exist and are used elsewhere (e.g. `AccountSelectionScreen`'s load-failure state), but are NOT wired into: `AccountSetupScreen._testConnection()`'s catch-all (`'Connection failed: $e'`, account_setup_screen.dart:238), `AccountSetupScreen._handleConnect()`'s credential-save failure (`'Failed to save credentials: $e'`, account_setup_screen.dart:299-301), and `startRealScan()`'s scan-failure catch (`'Scan failed: $e'`, scan_progress_screen.dart:696-712 -- the unmounted-context silent-log gap there is a KNOWN, already-documented tradeoff, not part of this item's scope).
  7. **Windows Environment Compatibility is genuinely unaudited** (window resizing, DPI scaling, common-hardware performance, UI flicker/jitter). The closest existing item, F63 (below), is superseded and scoped to ANDROID touch-target/density adaptation across 5 screens -- it says nothing about Windows resize/DPI behavior. This sub-item is an INVESTIGATION pass (manual resize/DPI-scale testing across the primary screens), not a known code fix -- time-box it and file findings as follow-up items rather than attempting fixes inline. (Confirmed the machine's actual DPI scale is 1.25/125% during the live walkthrough via WinWright's screenshot metadata -- a concrete data point for this sub-item, not yet a finding.)
- **Investigation Note -- true first-run state is hard to reproduce on a dev machine with existing installs.** Getting to a genuinely empty account/data state required discovering and working around THREE independent data-persistence layers that all needed clearing together: (1) the environment-scoped AppData directory (`MyEmailSpamFilter`/`MyEmailSpamFilter_Dev`), (2) a pre-existing legacy `com.example\spam_filter_mobile\` directory from the Sprint 19 app-identity change that a one-time migration (`app_identity_migration.dart`) still re-copies forward on every launch if the new-identity directory is ever empty, and (3) the account list itself, which lives in a single `saved_accounts` key via `flutter_secure_storage`'s Windows DPAPI-backed store (`flutter_secure_storage.dat`, inside the AppData directory but NOT touched by a naive directory rename alone if the migration re-seeds it from the legacy source first). All three were backed up (copy + verified byte-count match), moved aside together, tested against, then fully restored and re-verified byte-identical afterward -- no real account/rule/scan data was lost. This reproduction difficulty is itself worth a documentation note (e.g. in `docs/TESTING_STRATEGY.md` or a new `TROUBLESHOOTING.md` entry) so a future first-run test does not have to re-derive this.
- **Explicitly descoped from this item** (per Harold's Windows-only steering): any Android-specific verification or fix. Shared/provider-agnostic widget changes (e.g. the welcome screen, error-widget wiring, chip tooltips) will likely also render on Android once that track resumes, but this item's Definition of Done is Windows manual validation only -- do not claim Android coverage.
- **Not in scope**: MSIX installer-level UX (SmartScreen warnings, unsigned-package prompts) -- flagged as an open question in the First-Run Evaluation Summary Page's Section 1 but is a Store/Partner-Center-level concern, not an in-app code change; note as a possible future F# item if a real issue is found during Windows manual validation. The "Moved" feature itself (implementing move-on-match) is also out of scope here -- this item only removes the dead chip; implementing the feature is a separate future item if prioritized.
- Depends on: none. Independent of F150 (Android build blocker, deferred this sprint -- blocked on Harold's Firebase Console action).
- Sprint 58 scope: this is the ONLY item for Sprint 58 per Harold's explicit direction (2026-08-15).
- Source: Sprint 58 Backlog Refinement, Harold's user-centric MVP test script + First-Run Evaluation Summary Page, confirmed via live Windows walkthrough, 2026-08-15.

**F154. Help section for the Review No Rule Items screen (~45-60m) Priority 10 (Sprint 59)**
- Phase: Core App Quality / Help content
- Platform: All (content is shared; validated on Windows)
- Scope: new `HelpSection.reviewNoRuleItems` + `assets/content/help/review_no_rule_items.md` (ADR-0038 pattern: manifest entry, pubspec asset, validator pass). Content drafted from the ACTUAL screen functionality (`no_rule_review_screen.dart`) for Harold's review at Manual Validation. The screen's Help icon switches from the `resultsDisplay` nearest-match stand-in (gap noted in the screen's own code comment, deferred since F133-S52) to the new dedicated section. Adjust all help-link tests (enum count 22 -> 23, deep-link integration suite).
- Depends on: F155 (rename) lands first so the content uses the final name.
- Source: Harold, Sprint 59 Backlog Refinement, 2026-08-15. Full task card: `docs/sprints/SPRINT_59_PLAN.md` Task 4.

**F155. Rename "Review \"No Rule\" Items" -> "Review No Rule Items" app-wide, forward-looking (~20-30m) Priority 10 (Sprint 59)**
- Phase: Core App Quality / UX polish
- Platform: All (display strings shared)
- Scope: drop the awkward nested quotes from the display name everywhere CURRENT -- AppBar tooltip + screen title + all lib/ occurrences (13 across 10 files at planning), the 5 test files asserting the string (policy gates enforce the NEW name), `walkthrough.md`, and current backlog/process text. Identifiers/enums/file names unchanged (not user-visible, no quotes). Historical records (past sprint docs, CHANGELOG history) explicitly untouched per Harold: "no need to update historical records. Just from here forward."
...
**F153. Re-investigate Flutter Windows UIA/accessibility-tree exposure to WinWright -- [RESOLVED Sprint 59, 2026-08-15: F140's negative result was confounded; WinWright works. Full capability record in `docs/WINWRIGHT_SELECTORS.md`]**
- **Resolution (Sprint 59 re-test, flag enabled + tree primed from the start)**: F140's conclusion is refuted. (1) The empty-tree symptom was the missing `SPI_SETSCREENREADER` flag PLUS Flutter's lazy semantics init -- the first raw query after launch is empty even with the flag on; one `ww_get_snapshot` primes the tree and everything after sees full content. (2) `ww_invoke` (pure InvokePattern) works on Buttons. (3) `ww_scroll` direction mode works with correct direction semantics (~165px per call regardless of amount; long pages need many calls; live element bounds are the position probe). (4) Off-screen content is fully present in the tree with real bounds (`isOffscreen: true`), so READING never requires scrolling. (5) The F140 poster-child (Settings > General version text) is directly automation-readable. Residual gaps: `ww_scroll` into_view is broken on Flutter (RuntimeId `PropertyNotSupportedException` via handle; silent false-success via selector) and find_target is a tree lookup, not a scroll; `ww_dump_tree includePatterns` under-reports (test the verb, not the dump). Docs corrected: `WINWRIGHT_SELECTORS.md` (Prerequisites promoted to MUST-check pre-flight + F140 section superseded), F139's "Known gap" note (closed), `SPRINT_PLANNING.md` pre-flight (flag check + priming step). Upstream Flutter research (original scope below) NOT needed.
- Phase: Tooling / Testing Infrastructure
- Platform: Windows Desktop
- **Background**: F140 (Sprint 54, 2026-08-03) found a definitive NEGATIVE result at the time -- Flutter's Windows UIA bridge exposed ZERO control patterns to WinWright, for any element type, and no scrollable-region control type existed at all. This was treated as a Flutter-Windows-engine embedding limitation, not a usage gap.
- **BREAKTHROUGH (Sprint 58, F151 Task 7, 2026-08-15)**: `docs/WINWRIGHT_SELECTORS.md` documents a prerequisite that was apparently never actually satisfied during F140's original spike OR anywhere earlier in this sprint's live walkthrough: the Windows `SPI_SETSCREENREADER` system flag must be enabled (`mobile-app/scripts/enable-screen-reader-flag.ps1 enable`) before WinWright can see ANY Flutter semantics content. Checked during F151 Task 7 (`enable-screen-reader-flag.ps1 status` -> `False` -- confirmed NOT enabled), enabled it, and WinWright IMMEDIATELY began returning full semantics trees (31+ interactive elements with real bounds), successfully **clicking real app buttons** via `ww_invoke`/selector (Settings, Manual Scan tab navigation all worked), and a `ww_scroll` call **visibly moved real Help-screen content** -- directly contradicting F140's "no scroll patterns exposed at all" finding.
- **What this means**: F140's negative result and this sprint's earlier "F151 walkthrough confirmed the same empty-snapshot finding" note (now understood to be the SAME confounded measurement, not independent confirmation) may have been entirely an artifact of a missing one-time OS flag, not a genuine Flutter-Windows engine ceiling. The true state of Flutter's Windows UIA support is UNKNOWN again and needs a clean re-test with the flag correctly enabled from the start of the test, not assumed.
- **Scope for this re-run** (supersedes F153's original scope below -- do this FIRST, it is far cheaper than an upstream research pass):
  1. Confirm `enable-screen-reader-flag.ps1 status` at the START of any WinWright session -- if `False`, enable it BEFORE drawing any conclusion about what WinWright can or cannot see.
  2. Re-run F140's original spike test (does `ww_get_snapshot`/`ww_dump_tree` with `includePatterns: true` see actual Flutter widget content and real UIA control patterns, not just window chrome) with the flag confirmed enabled.
  3. Re-test scroll specifically (`ww_scroll` in all 3 modes: direction, into_view, find_target) against a real long-scrolling screen (e.g. Help) to determine if scroll is now fully reliable, partially working (as the one Sprint 58 data point suggests -- direction control was unclear), or still genuinely absent.
  4. If Flutter Windows UIA support turns out to be fully real once the flag is enabled: this changes F139's "Known gap" note (About-screen version text unreachable by automation) and potentially other WinWright-limitation workarounds recorded elsewhere (`docs/WINWRIGHT_SELECTORS.md`'s F140 section, `settings_screen.dart`/`help_screen.dart`'s "duplicate near top" pattern) -- these do not need to be UNDONE (duplicating content near the top is harmless and still good UX), but the DOCUMENTED REASON for them should be corrected so future sessions do not inherit a wrong belief about what WinWright can do.
  5. Investigate WHY the flag was not enabled earlier -- is there a missing pre-flight step in the sprint process (`SPRINT_PLANNING.md`'s Tooling-Capability Pre-Flight guidance) that should check for this before any WinWright-based investigation begins? If so, that is a process fix, not an app fix.
- **Original scope (if the above confirms Flutter genuinely still has gaps even with the flag enabled)**:
...
- **How to use**: this item is no longer a clean periodic-review template until the re-test above resolves the flag-confound question -- treat as an active, prioritized investigation for the next sprint that can execute it, not HOLD.
- Source: Harold, 2026-08-15, during F151's live Windows walkthrough -- flagged as unreasonable that no real fix exists and requested a fresh backlog item to re-investigate; the re-investigation itself (F151 Task 7) found the likely root cause the same day.
