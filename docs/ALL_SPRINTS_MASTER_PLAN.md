...
**Last Updated**: 2026-08-15 (**0.7.0.0 CERTIFIED AND LIVE on the Microsoft Store** (Submission 14, 2026-08-15) -- first MINOR release under the enforced semver policy, triggered by Sprint 57's F142 `feat` entry. Check C smoke pass confirmed clean (installed-build screenshot: title bar + Settings > General both show no `[DEV]` marker, version reads 0.7.0). Built from a genuinely current `main` (real develop->main merge via PR #316 already in place) -- no local-unpushed-merge workaround needed this cycle, closing out the 0.6.2.0-era known gap for good. Dev worktree bumped 0.7.0 -> 0.7.1 (PATCH, `[Unreleased]` empty) post-cert. Sprint 57 (F142 Android navigation model + F149 safe-sender AOL Inbox/Bulk oscillation fix) MERGED to develop (PR #314) and main (PR #316, Harold); Manual Validation complete; GitHub Copilot review on PR #314 -- 6 findings, all fixed; retro all 14 categories Very Good, 1 improvement applied. Sprint 58 branch created (`feature/20260814_Sprint_58`); light Backlog Refinement done -- **F150** (Android `google-services.json`/`applicationId` mismatch blocking all Android builds) confirmed the sole unblocked candidate, its fix step needs Harold's Firebase Console action; F143/F144 remain available HOLD candidates for a future Android-focused sprint.)
