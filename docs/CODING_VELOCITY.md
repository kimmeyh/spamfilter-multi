...
| S46-F39-FIX: unmatched_emails had NO production writer (manual-testing round 3) | 46 | DONE | [no-est, reactive] | -- | ~35 | ~35 | Review screen showed 0 items vs no_rule_count 2/1/1. Root cause: Sprint 4 placeholder only logged "will persist in Task D" -- table never written; F39 widget tests seeded it directly, masking the gap. Batch write added in _persistEmailActions (one site covers manual+background), F96 auth snapshot included, SEC-14 body excluded; regression test pins it. LESSON for retro: when building on an existing store, verify the WRITE path exists in production, not just the store API. |
| S46-IMP-1: provider-sender grouping in all scan-result lists (retro) | 46 | DONE | 45-75 | 45-75 | ~55 | ~55 | Shared ProviderSenderGrouping utility + header/end widgets; wired into ResultsDisplayScreen (all filters) + NoRuleReviewScreen; 10 tests. Within estimate. |
| S46-IMP-2: shared db-widget-test harness (retro) | 46 | DONE | 20-30 | 20-30 | ~15 | ~15 | mountAndLoadDbWidget + documented rules; 2 usage sites refactored; 9 tests green unchanged. Under estimate. |
| S46-IMP-5: -SkipClean root cause + fallback (retro) | 46 | DONE | 10-15 | 10-15 | ~20 | ~20 | Deeper root cause than proposed: mandatory native_assets deletion makes -SkipClean deterministically broken (not flaky); now warns + falls back to full clean. Slightly over estimate due to the deeper diagnosis. |
