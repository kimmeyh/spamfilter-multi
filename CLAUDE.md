...
- **Don't use bare `git add -A` / `git add .` in sprint commits.** Stage explicit paths, or run `git status --short` first and confirm every entry belongs. (Sprint 52 IMP-4: `git add -A` swept an unrelated working file into a commit. It happened to be correct to include under the `0*` rule -- correct by luck, not intent. The failure mode is a commit whose contents nobody chose.)
